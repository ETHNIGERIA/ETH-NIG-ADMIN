...
import { pitchApplicationsToCsv } from '@/tickets-portal/lib/pitch-csv';
import type { PitchApplication } from '@/tickets-portal/types/admin-pitch-application';

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

const FILTER_KEYS = ['status', 'sector', 'stage', 'slot', 'tier', 'from', 'to', 'q'];
...
export async function GET(req: NextRequest) {
  ...
  const result = await fetchAllAdminPages<PitchApplication>(
    token,
  ...
  return new Response(pitchApplicationsToCsv(result.rows), {
    headers: {
      'Content-Type': 'text/csv; charset=utf-8',
      'Content-Disposition': `attachment; filename="pitch-applications-${stamp}.csv"`,
