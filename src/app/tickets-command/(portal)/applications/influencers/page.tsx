...
export default async function InfluencerApplicationsPage({ searchParams }: { searchParams: Promise<ListSearchParams> }) {
  ...
  try {
    const data = await ticketsApiGet<ApplicationPage<InfluencerApplication>>(
      `/admin/influencer-applications${toQuery({ page, limit: ADMIN_PAGE_SIZE, status })}`,
    );
