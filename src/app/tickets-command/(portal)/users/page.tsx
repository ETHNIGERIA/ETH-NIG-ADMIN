...
export default async function UsersPage({
  ...
  try {
    result = await ticketsApiGet<Paginated<AdminUser>>(`/admin/users?page=${page}&limit=${limit}`);
  } catch (e) {
  ...
  const { data: users, pages, total } = result;

  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <header className="space-y-1">
          <h1 className="text-[28px] font-semibold tracking-tight text-stone-900">Users</h1>
                ...
                return (
                  <tr key={id} className={`border-b border-stone-100 last:border-0 ${rowHover}`}>
                    <td className={`${td} font-medium text-stone-900`}>
                      <Link href={`/tickets-command/users/${id}`} className="hover:underline">
                        {user.email}
                      </Link>
                    </td>
      ...
      {pages > 1 ? (
        <div className="flex items-center justify-between gap-4 text-[14px] text-stone-600">
          ...
          <div className="flex gap-2">
            {page > 1 ? (
              <Link
                href={`/tickets-command/users?page=${page - 1}`}
